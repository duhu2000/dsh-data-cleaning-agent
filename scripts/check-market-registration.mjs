...
const DEFAULTS = {
  ...
  expectedName: 'dsh-data-cleaning-agent',
  expectedRepository: 'https://github.com/duhu2000/dsh-data-cleaning-agent',
  registrationUrl:
    'https://raw.githubusercontent.com/awesome-dsh-plugin/awesome-dsh-plugin/main/data/plugins/duhu2000__dsh-data-cleaning-agent.yml',
...
export function assessRegistration({ pullRequest, registrationText, registry, expectedName, expectedRepository }) {
  ...
  if (pullRequest.merged_at) {
    ...
    return {
      ...
      plugin: plugin
        ? { name: plugin.name, url: plugin.url ?? plugin.repository ?? plugin.repo, npm: plugin.npm }
        : null,
    };
  }
...
async function fetchResponse(url, { token, json = false } = {}) {
  const headers = {
    ...
    'user-agent': 'dsh-data-cleaning-agent-market-check',
  };
...
function summaryMarkdown(result, options) {
  ...
  if (options.pullRequestNumber) {
    lines.push(`- PR：https://github.com/${options.pullRequestRepository}/pull/${options.pullRequestNumber}`);
  } else {
    lines.push('- PR：尚未提交（提交后设置 `DSH_MARKET_PR_NUMBER` 仓库变量）');
  }
  lines.push(
...
export async function runMarketRegistrationCheck({ env = process.env, args = process.argv.slice(2) } = {}) {
  const pullRequestNumber = Number(env.MARKET_PR_NUMBER || DEFAULTS.pullRequestNumber || 0);
  const options = {
    pullRequestRepository: env.MARKET_PR_REPOSITORY || DEFAULTS.pullRequestRepository,
    pullRequestNumber: Number.isInteger(pullRequestNumber) && pullRequestNumber > 0 ? pullRequestNumber : null,
  ...
  if (options.pullRequestNumber) {
    ...
    pullRequest = await fetchResponse(apiUrl, { token: env.GITHUB_TOKEN, json: true });
    if (pullRequest.merged_at) {
  ...
  console.log(JSON.stringify({
    ...
    pullRequest: options.pullRequestNumber
      ? `https://github.com/${options.pullRequestRepository}/pull/${options.pullRequestNumber}`
      : null,
  }, null, 2));
