...
test('规范化 GitHub 仓库地址', () => {
  assert.equal(
    normalizeRepository('git+https://github.com/duhu2000/dsh-data-cleaning-agent.git'),
    expected.expectedRepository,
  );
  assert.equal(normalizeRepository(`${expected.expectedRepository}/`), expected.expectedRepository);
});

test('兼容官网 plugins.json 与 DSH Market 包装格式', () => {
  const plugin = { name: expected.expectedName, url: expected.expectedRepository };
  assert.deepEqual(registryPlugins([plugin]), [plugin]);
...
test('尚未提交 PR 时保持等待且不误报失败', () => {
  const result = assessRegistration({ pullRequest: null, ...expected });
...
test('PR 已合并但目录未同步时进入待同步状态', () => {
  const result = assessRegistration({
    pullRequest: { state: 'closed', merged_at: '2026-09-02T00:00:00Z' },
...
test('YAML 和 plugins.json 均生效后验收通过', () => {
  const result = assessRegistration({
    pullRequest: { state: 'closed', merged_at: '2026-09-02T00:00:00Z' },
    registrationText: `name: ${expected.expectedName}\nurl: ${expected.expectedRepository}.git`,
    registry: {
      plugins: [{ name: expected.expectedName, url: expected.expectedRepository, npm: expected.expectedName }],
    },
    ...expected,
  });
