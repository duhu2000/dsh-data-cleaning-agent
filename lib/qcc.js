...
export function mapRegistrationFields(value, fallback = {}) {
  ...
  return {
    ...
    region: text('所属地区'),
    province: coreFieldText(value.地区信息?.省份),
    city: coreFieldText(value.地区信息?.城市),
    district: coreFieldText(value.地区信息?.区域),
    area_code: coreFieldText(value.地区信息?.地区代码),
    business_scope: text('经营范围'),
    industry_category: industryDisplay(value.国标行业, ['门类', '大类', '中类', '小类']),
    industry_section: coreFieldText(value.国标行业?.门类),
    industry_large: coreFieldText(value.国标行业?.大类),
    industry_middle: coreFieldText(value.国标行业?.中类),
    industry_small: coreFieldText(value.国标行业?.小类),
...
// Explicit leaves only: old flat text must not be guessed into industry levels.
function coreFieldText(value) {
  return typeof value === 'string' || typeof value === 'number' ? String(value) : '';
}

function industryDisplay(value, levels) {
  if (!isRecord(value)) return coreFieldText(value);
  return levels.map(level => {
    const text = coreFieldText(value[level]);
    return text.trim() ? `${level}：${text}` : '';
  }).filter(Boolean).join('；');
}
...
export function mapProfileFields(value) {
  ...
  return {
    ...
    qcc_industry_level1: coreFieldText(value.企查查行业?.一级),
    qcc_industry_level2: coreFieldText(value.企查查行业?.二级),
    qcc_industry_level3: coreFieldText(value.企查查行业?.三级),
    qcc_industry_level4: coreFieldText(value.企查查行业?.四级),
    main_products: Array.isArray(value.主营产品)
      ? value.主营产品.filter(item => typeof item === 'string' && item.trim()).join('；')
      : coreFieldText(value.主营产品),
    company_scale: coreFieldText(value.企业规模),
