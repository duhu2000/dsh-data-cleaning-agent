...
`;
    ...
    function mappingRecommendations(headers, fields = [...ENRICHMENT_OPTIONS, ...INPUT_ONLY_MAPPING_OPTIONS]) {
      const rows = (Array.isArray(headers) ? headers : []).map((sourceField) => {
        const label = normalizeMappingLabel(String(sourceField).replace(/（重复列 \d+）$/, ''));
    ...
    function MappingPicker({ sourceField, mappings, groups, recommendation, onChange }) {
      ...
      const occupied = (id) => mappings.some((item) => item.sourceField !== sourceField && item.targetField === id);
      ...
      return h('div', { className: 'dcAgentMappingRow' },
        ...
        h('div', { style: { minWidth: 0 } },
          ...
          h('small', { className: 'dcAgentHint' }, value ? `已映射：${label}；可手动调整` : recommendation.reason),
    ...
    function projectCompletionResult({ rows = [], headers = [], mappings = [], fieldSelection = [] } = {}) {
      ...
      const pairs = mappings.filter((item) => sourceHeaders.includes(item.sourceField) && selected.includes(item.targetField)
        ...
        && mappings.filter((other) => other.targetField === item.targetField).length === 1);
      ...
      return defineStore({
        ...
        actions: {
          ...
          setMapping: (draft, sourceField, targetField) => {
            if (targetField && draft.mappings.some((item) => item.sourceField !== sourceField && item.targetField === targetField)) return;
            const mappings = draft.mappings.filter((item) => item.sourceField !== sourceField);
      ...
      if (step === 'history') {
        ...
        pane = h('div', { className: 'dcAgentPane' },
          ...
          h('section', { className: 'dcAgentSection' },
            ...
            h('p', { className: 'dcAgentHint' }, `已映射 ${mappings.length} / ${dataset?.headers?.length || 0} 列。标准字段名和明确别名自动匹配；多义表头、重复列请确认推荐。仅在本机匹配表头，不调用模型或企查查，不改变下方输出字段。`),
