...
`;
    ...
    const CORE_FIELD_GUIDE = '可分别选择省市区、地区代码、四级国标行业与四级企查查行业；未分级行业列保留完整层级展示。企业规模不等于人员规模；主营产品按原顺序合并一格（上游最多返回 10 项，非完整产品库）。缺失值留空，不推测。';
    ...
    function buildTaskPrompt(config = {}) {
      ...
      const source = mode === 'image'
        ? config.imageState === 'completed'
          ? `图片${config.fileName ? `「${config.fileName}」` : ''}，已由企查查智能文档解析识别 ${entryCount} 条主体标识`
      ...
      if (mode === 'image' && config.imageCommandId && config.imageState !== 'completed') {
        lines.push('本次发送即授权使用本人企查查账号额度，按向导已确认范围执行图片识别、质量体检、主体匹配和字段补全，生成新 Excel。识别后自动沿用已选规则与字段，不重复确认或要求二次发送；仅识别歧义、多候选或权限异常时暂停，模糊字符不得猜测。');
    ...
    const MAPPING_ALIASES = {
      province: ['省', '所属省份'], city: ['市', '所属市', '所属城市'],
      district: ['区县', '所属区县', '所属区域'], area_code: ['区域编码', '地区编码', '行政区划代码'],
      industry_section: ['国标行业一级', '国标行业门类名称'],
      industry_large: ['国标行业二级', '国标行业大类名称'],
      industry_middle: ['国标行业三级', '国标行业中类名称'],
      industry_small: ['国标行业四级', '国标行业小类名称'],
      qcc_industry_level1: ['企查查一级行业', '企查查行业一级分类'],
      qcc_industry_level2: ['企查查二级行业', '企查查行业二级分类'],
      qcc_industry_level3: ['企查查三级行业', '企查查行业三级分类'],
      qcc_industry_level4: ['企查查四级行业', '企查查行业四级分类'],
      main_products: ['主要产品'], company_scale: ['企业规模分类'],
    ...
    async function createWorkflowTask(actions, sessionId, draft = {}) {
      ...
      if (!pending) {
        pending = requestJson('/data-cleaning/api/workflow/tasks', 'POST', {
          ...
          ...(draft.source?.type === 'image' ? { source: draft.source } : {}),
        });
    ...
    async function stageConfirmedWorkflow(actions, sessionId, expected = null) {
      ...
      if (expected?.mode === 'image' && expected.imageState === 'prepared') {
    ...
    function PromptGenerator(props) {
      ...
      // 独立预览不依赖原生附件的 blob URL；发送前移除附件可能撤销该 URL。
            ...
            if (command.taskId) {
              ...
              if (actions) {
                ...
                if (command.qccCommand) {
                  ...
                  actions.setCommandProgress?.({ taskId: command.taskId, ...(status.command?.progress || {}) });
                }
              }
              if (command.state === 'completed') {
                const recognized = command.result?.entries || [];
                setEntries(recognized); setEntryCount(recognized.length);
                const result = entriesToDataset(recognized);
                const runtime = runtimeFor(command.taskId);
                runtime.rows = result.rows; runtime.sourceRows = result.rows; runtime.headers = result.headers;
                runtime.source = { type: 'image', fileName: imageFileName, sizeBytes: imageSizeBytes };
                if (actions) actions.setDataset(result);
                setOpen(false);
                return;
              }
            }
      ...
      const handleImageFile = async (file) => {
        ...
        try {
          ...
          const previewUrl = `data:${file.type};base64,${await imageFileBase64(file)}`;
          if (imageAttachment && typeof removeImage === 'function') removeImage(sessionId, imageAttachment);
      ...
      return h('div', { className: 'dcAgentPromptLayer' },
        ...
        open ? h(PromptDialog, { sessionId, onClose: () => setOpen(false) },
          ...
          wizardStep === 3 ? h('div', { className: 'dcAgentWizardPane' },
            ...
            h('p', { className: 'dcAgentHint' }, CORE_FIELD_GUIDE),
        ...
        imageLightbox && imagePreviewUrl ? h(ImagePreview, {
          src: imagePreviewUrl, alt: imageFileName, sessionId, onClose: () => setImageLightbox(false),
        }) : null,
      );
    }
    ...
    function DataToolCard(props) {
      ...
      return h('div', { className: 'dcAgentToolCard', 'data-tool': toolName, 'data-state': state },
        ...
        ['data_cleaning_qcc_run', 'data_cleaning_extract_image_companies'].includes(toolName) ? h('div', { className: 'dcAgentArtifactList' },
        ...
        if (!canAction('edit-rules') && !canAction('confirm-rules')) {
          ...
          h('section', { className: 'dcAgentSection' },
            ...
            h('div', { className: 'dcAgentRow', 'aria-label': '补全字段批次概览' },
              h('span', { className: 'dcAgentChip' }, `工商与简介 ${uiFieldGroups.filter(([id]) => ['company_registration', 'company_profile'].includes(id)).reduce((sum, [, , fields]) => sum + fields.length, 0)} 项`),
              h('span', { className: 'dcAgentChip' }, `共 ${targetFields.filter(([id]) => id !== 'phone').length} 项可选字段`),
            ),
            h('p', { className: 'dcAgentHint' }, CORE_FIELD_GUIDE),
    ...
    function installSessionWorkbench(ctx) {
      ...
      const previewTabId = 'dsh-data-cleaning-agent:artifact-preview';
      const previewUrls = new Map();
      const previewListeners = new Set();
      function ArtifactPreviewTab(props) {
        const url = react.useSyncExternalStore(listener => {
          previewListeners.add(listener);
          return () => previewListeners.delete(listener);
        }, () => previewUrls.get(props.scope.sessionId) || '', () => '');
        return url ? h('iframe', { src: url, title: '清洗结果 Excel 预览',
          sandbox: 'allow-same-origin allow-downloads',
          style: { display: 'block', width: '100%', height: '100%', minHeight: '320px', border: 0 } })
          : h('p', null, '请点击清洗结果文件的「预览 / 打开」。');
      }
      const previewClick = event => {
        ...
        if (!anchor || !previewService) return;
        let url;
        try { url = new URL(anchor.href, location.href); } catch { return; }
        if (url.origin !== location.origin || url.search !== '?preview=1'
          || !/^\/data-cleaning\/api\/workflow\/tasks\/dcw-[a-zA-Z0-9-]+\/artifacts\/dca-[a-zA-Z0-9-]+$/.test(url.pathname)) return;
        const sessionId = previewService.getSnapshot().sessionId;
        if (!sessionId || !previewService.isTabEnabled(previewTabId)) return;
        previewUrls.set(sessionId, url.pathname + url.search);
        try { previewService.openTab({ type: previewTabId }, { sessionId }); }
        catch { return; } // Native browser link remains usable if the provider cannot open.
        event.preventDefault();
        event.stopPropagation();
        for (const listener of previewListeners) listener();
      };
      host.addEventListener?.('click', previewClick, true);
      ...
      const connectProvider = service => {
        ...
        adapter.dispose();
        adapter = next;
        const unregisterPreview = next.compatible ? service.registerTab({
          id: previewTabId, title: '清洗结果预览', order: 31, single: true,
          icon: size => h(DatabaseLogo, { size }), component: ArtifactPreviewTab,
        }) : () => {};
        previewService = next.compatible ? service : null;
        return () => {
          unregisterPreview();
          if (previewService === service) previewService = null;
          next.dispose();
      ...
      const listeners = {
        ...
        [WORKBENCH_PREPARE_EVENT]: event => {
          ...
          queueWorkflowOperation(sessionId, () => stageConfirmedWorkflow(actions, sessionId, config))
            .then(command => { if (!disposed && command.commandId?.startsWith('dcq-')) watchHostedCommand(actions, sessionId, command); resolve(command); }, reject);
        },
      };
      ...
      const shared = { controller, references: 1, release() {
        ...
        previewUrls.clear(); previewListeners.clear();
        adapter.dispose();
        disconnectFixture();
