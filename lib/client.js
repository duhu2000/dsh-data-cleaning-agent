...
`;
    ...
    function buildTaskPrompt(config = {}) {
      ...
      if (mode === 'excel' && config.headers?.length) {
        ...
        lines.push(`原表共 ${config.headers.length} 列。确认写回：${pairs.map((item) => `「${item.sourceField}」←${resultFieldLabel(item.targetField)}`).join('；') || '暂无'}。`);
    ...
    function requestWorkbenchDraft(draft, sessionId) {
      ...
      if (rootWorkbenchActions) {
        ...
        if (draft.mappings) rootWorkbenchActions.setMappings(draft.mappings);
    ...
    function WorkbenchDrawer(props) {
      ...
      const prepareEditableQccCommand = async (payload) => {
        ...
        const host = await api('/data-cleaning/api/g5/capabilities');
