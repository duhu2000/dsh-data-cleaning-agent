 ...
 *
 * M2 工作台复用既有可信后端 `/data-cleaning/api/mvp/*`（parse / profile / clean /
 * complete），只做「表单 + 调用 + 预览 + 下载」薄壳；企查查补全本轮不接调用链，
 * 仅显示只读状态位（对应方案 §5.4 的 mockup 高保真映射）。
 */
...
`;
    ...
    let lastCsv = { clean: null, complete: null };

    const STEPS = [
      { key: 'upload', label: '上传与映射', icon: '📄' },
      { key: 'profile', label: '数据体检', icon: '🩺' },
      { key: 'clean', label: '清洗补全', icon: '🧹' },
      { key: 'export', label: '导出', icon: '⬇️' },
    ];
    ...
    function WorkbenchOverlay(props) {
      ...
      if (step === 'profile') {
        pane = h('div', { className: 'dcAgentPane' },
          ...
          profile ? h('div', null,
            ...
            h('div', { className: 'dcAgentRow' },
              h('button', { type: 'button', className: 'dcAgentButton is-primary', disabled: busy, onClick: () => actions.setStep('clean') }, '下一步：清洗补全'),
            ),
          ) : h('div', { className: 'dcAgentRow' },
        ...
        pane = h('div', { className: 'dcAgentPane' },
          h('p', { className: 'dcAgentHint' }, '本地确定性清洗与补全：去重、非法金额剔除、缺失金额补 0、名称/手机号归一。差异预览仅反映本地规则，非企查查分数。'),
          h('div', { className: 'dcAgentRow' },
            h('button', { type: 'button', className: 'dcAgentButton', disabled: busy, 'aria-label': '执行清洗', onClick: runClean }, busy ? '处理中…' : '清洗'),
            h('button', { type: 'button', className: 'dcAgentButton', disabled: busy, 'aria-label': '执行补全', onClick: runComplete }, busy ? '处理中…' : '补全'),
          ),
          ...
          complete ? h('div', null,
            h('p', { className: 'dcAgentHint' }, '补全摘要'),
            h('div', { className: 'dcAgentGrid' },
              stat('总数', complete.total), stat('已补全', complete.completed, 'good'), stat('仍缺失', complete.incompleteCount, complete.incompleteCount > 0 ? 'warn' : null),
              stat('补名称', complete.name), stat('补金额', complete.amount), stat('手机号归一', complete.phoneNormalized),
            ),
          ) : null,
          (clean || complete) ? h('div', { className: 'dcAgentRow' },
            h('button', { type: 'button', className: 'dcAgentButton is-primary', onClick: () => actions.setStep('export') }, '下一步：导出'),
          ) : null,
        );
      } else if (step === 'export') {
        pane = h('div', { className: 'dcAgentPane' },
          h('p', { className: 'dcAgentHint' }, '下载清洗/补全后的 CSV。结果由本地确定性引擎产生，未调用企查查。'),
          h('div', { className: 'dcAgentGrid' },
            stat('输入行数', dataset ? dataset.rowCount : '—'),
            stat('清洗保留', clean ? clean.kept : '—', clean && clean.kept > 0 ? 'good' : null),
            stat('补全完成', complete ? complete.completed : '—', complete && complete.completed > 0 ? 'good' : null),
          ),
          h('div', { className: 'dcAgentRow' },
            h('button', { type: 'button', className: 'dcAgentButton is-primary', disabled: !lastCsv.clean, 'aria-label': '下载清洗结果', onClick: () => download('clean') }, '下载清洗结果 CSV'),
        ...
        pane = h('div', { className: 'dcAgentPane' },
          h('p', { className: 'dcAgentHint' }, '上传 CSV / XLSX / XLS / JSON，或粘贴数据。原始数据仅在本机解析，不进入模型上下文；企查查补全本轮不接调用链。'),
          h('input', {
            type: 'file',
      ...
      return h('div', {
        ...
        h('div', { className: 'dcAgentWorkbench' },
          h('header', { className: 'dcAgentWbHeader' },
            h('div', { className: 'dcAgentWbTitle' },
              h('span', { className: 'dcAgentWbIcon', 'aria-hidden': 'true' }, '🧹'),
              h('div', null,
                h('b', null, '数据清洗补全'),
                h('small', null, '本地确定性清洗 · 企查查补全只读'),
              ),
            ),
            h('span', { className: 'dcAgentQccBadge', title: '本轮不接入企查查调用链' }, '企查查补全 · 只读'),
