...
export class DataCleaningWorkflowStore {
  ...
  async create(input = {}) {
    ...
    const record = {
      ...
      source: input.source?.type === 'image' ? sanitizeSource({ ...input.source, rowCount: 0 }) : null,
      qualitySummary: null,
      matchSummary: null,
  ...
  async updateDraft(id, input = {}) {
    return this.mutate(id, input.expectedRevision, (record) => {
      ...
      return { ...draft, ...(record.state === 'draft' && input.source?.type === 'image'
        ? { source: sanitizeSource({ ...input.source, rowCount: 0 }) } : {}) };
    });
  }
