...
function publicRecord(record, now = Date.now()) {
  return structuredClone({
    ...
    taskId: record.workflow?.taskId,
    qccCommand: record.qccCommand,
    error: record.error,
  });
}

function safeFailure(error) {
  if (['WorkflowError', 'QccBridgeError'].includes(error?.name)) {
  ...
  async run(commandId, exec) {
    ...
    record.promise = execute(provider.parseName, {
      ...
      record.result = {
        ...
        needsReview: /[�？?]|无法辨认|识别不清|待确认/.test(text),
      };
      if (record.workflow && this.continueWorkflow) {
        record.result.workflowResult = await this.continueWorkflow(record, record.result, exec);
      }
...
export function registerImageIntakeTool(tools, store) {
  return tools.register({
    ...
    description: 'Execute one staged company-list image task using a visible dci-* commandId. The Host parses the image once. If the wizard bound an explicitly confirmed execute-on-send workflow, the same Agent-owned call continues quality checks, entity matching, selected-field enrichment and XLSX delivery. Ambiguous recognition or entity candidates require review; never call underlying MCP tools separately or repeat the task.',
    parameters: {
      type: 'object',
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          workflowResult: { type: 'object', additionalProperties: true },
        },
        required: ['commandId', 'entries', 'entryCount', 'truncated'],
      },
      render: (_args, value) => [{
        type: 'text',
        text: value.workflowResult
          ? `图片识别及已确认任务已执行，识别 ${value.entryCount} 条。真实执行统计：${JSON.stringify(value.workflowResult)}。${value.workflowResult.reviewReason || '无需重复确认规则或再次发送；仅按真实异常提示处理。'}\n${(value.workflowResult.artifacts || []).map(a => `${String(a.fileName).replace(/[^a-zA-Z0-9\u4e00-\u9fff._ -]/g, '_')}：[预览 / 打开](${a.url}?preview=1) · [下载 Excel](${a.url})`).join('\n')}`
          : `图片已识别 ${value.entryCount} 条，等待核验。此图片未绑定已确认的补全任务，请在「导入与核验」核验后配置后续处理；识别名单不代表已匹配。`,
      }],
    },
