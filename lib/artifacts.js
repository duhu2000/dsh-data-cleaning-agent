...
import { projectCompletionResult } from './engine.js';
...
export class WorkflowArtifactStore {
  ...
  async createBundle(taskId, input = {}) {
    ...
    const projected = projectCompletionResult({
      rows: normalizeRows(input.rows), headers: input.headers || [],
      mappings: input.mappings || [], fieldSelection: normalizeFieldSelection(input.fieldSelection),
    });
    const rows = projected.rows;
    const headers = normalizeHeaders(projected.headers, rows);
