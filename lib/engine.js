...
// 重复表头必须获得稳定、无碰撞的键，否则行对象会覆盖前一列原值。
function uniqueHeaders(values) {
  const bases = Array.from(values, (value, i) => String(value ?? '').trim() || `col_${i + 1}`);
  const reserved = new Set(bases);
  const used = new Set();
  return bases.map((base) => {
    let key = base;
    let ordinal = 2;
    // 原表也可能包含同名的重复列标记，不能覆盖它。
    while (used.has(key) || (key !== base && reserved.has(key))) key = `${base}（重复列 ${ordinal++}）`;
    used.add(key);
    return key;
  });
}

/** 原列补空投影：由已确认映射决定目的列，非空原值（包括 0/false）不覆盖。 */
export function projectCompletionResult({ rows = [], headers = [], mappings = [], fieldSelection = [] } = {}) {
