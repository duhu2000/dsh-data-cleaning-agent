...
export function registerEnrichSkill(skills) {
  return skills.register({
    ...
    content: [
      'UX-49 最高优先级：先检查本业务请求的信息是否充分，再考虑以下任何工具或执行规则。若仍含未补全的【】占位符，或没有实际企业名单/Excel/名单图片与待补字段，先用中文要求提供名单或文件及待补字段；零工具调用，包括 OAuth 检查、体检、OCR、MCP、创建 workflow。例子、说明、【企业名单】均不是企业。用户改过的初始模板按普通内容理解，不得整体忽略。有真实暂存来源和已选字段的安全任务凭证可提供这些信息，不要求重复提供。此规则只作用于数据清洗补全业务，不得全局拦截 DSH 发送或其它产品。',
