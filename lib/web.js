...
export function mountWebRoutes(wctx, { logger, report, TOOL_NAME, SKILL_NAME }) {
    ...
    try {
      ...
      if (rest.length === 1 && req.method === 'PATCH') {
        ...
        if (payload.executeOnSend !== true || record.state !== 'prepared' || task.state !== 'draft'
          ...
          throw new ImageIntakeError('DC_IMAGE_CONFIG_CONFLICT', '图片或任务配置已变更，或未选择补全字段，请重新确认。', 409);
        }
        let artifactOrigin = '';
        ...
        if (typeof requestHost === 'string' && /^[a-zA-Z0-9.:[\]-]+$/.test(requestHost)) {
          ...
          } catch { /* Use relative artifact links. */ }
        }
        record.workflow = { taskId: task.id, revision: task.revision, artifactOrigin };
        return writeJson(res, 200, { ok: true, command: imageIntake.status(record.commandId) });
      }
  ...
  register('/data-cleaning/api/workflow/tasks', async (req, res) => {
    ...
    try {
      ...
      if (rest.length === 2 && rest[1] === 'verify-source') {
        if (req.method !== 'POST') {
          return writeJson(res, 405, { ok: false, code: 'DC_METHOD', message: 'Source verification requires POST.' });
        }
        const body = await readBody(req);
        const payload = body.length ? JSON.parse(body.toString('utf8')) : {};
        const current = await workflow.assertRequestOrigin(taskId, payload);
        const verified = Boolean(current.source?.checksum)
          && sourceVerifier(payload.headers, payload.rows, current.source.checksum) === current.source.checksum;
        return writeJson(res, 200, {
          ok: true,
          marker: 'data-cleaning-source-verifier-v1',
          verified,
        });
      }
