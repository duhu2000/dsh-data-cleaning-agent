    ...
    try {
      ...
      if (payload?.confirmPaidCalls !== true) {
        return writeJson(res, 409, {
          ...
          message: 'Set confirmPaidCalls=true after the user confirms QCC paid data calls.',
        });
      }
      const rows = Array.isArray(payload?.rows) ? payload.rows : [];
      const result = await qccBridge.enrichRows(rows, {
        nameField: String(payload?.nameField ?? 'name'),
        includeRisk: payload?.includeRisk === true,
        concurrency: Number(payload?.concurrency ?? 2),
        maxRows: 100,
      });
      const requestedHeaders = Array.isArray(payload?.headers) ? payload.headers.map(String) : [];
      ...
      writeJson(res, 200, {
        ...
        rows: result.rows,
        csv: toCsv(headers, result.rows),
        downloadName: 'qcc-enriched.csv',
      });
    } catch (error) {
      const code = error?.code ?? 'QCC_BRIDGE';
      const status = code === 'QCC_NOT_CONNECTED' || code === 'QCC_TOOL_UNAVAILABLE'
        ? 503
        : code === 'QCC_TIMEOUT' ? 408 : code === 'QCC_ABORTED' ? 409 : 400;
      const payload = error instanceof QccBridgeError
        ? error.toJSON()
        : { code, message: error instanceof Error ? error.message : String(error) };
      writeJson(res, status, { ok: false, ...payload });
    }
  });
