...
`0.5.2 UI Alignment` 已正式发布：npm `latest`、Git tag 与 GitHub Latest 一致。`v0.5.2` 指向
`a1bb68a`；Release workflow `33722000758` 全绿，npm provenance 为 SLSA v1，公共 Registry
全新安装、ESM 导出与包内双语 README 均已验证。0.5.0 的 P1.1～P1.6、最小真实 Phase-3 E2E
结论保持有效。
P0/G3 市场收录已全部门完成（PR 合并 + 目录 YAML 落库 + `plugins.json` 命中 + 视觉市场一键安装冒烟通过）。

入口位于「新会话 / 工作区」之间，中央复用原生会话和五能力入口，右侧为非模态工作台；解析与体检
流程通过。DSH `0.1.1-rc.2` / `0.1.2-alpha.2` 全新隔离安装、代码审查、严格发布门和公共安装均通过。
...
本地与远端 `main` 包含 0.5.2 UI Alignment 代码、测试和发布记录；npm `latest` 与 GitHub Latest 均为 0.5.2。
...
| 项 | 值 |
| --- | --- |
| 代码基线 | `v0.5.2` → `a1bb68a`；`main` / `origin/main` 发布后文档收口 |
| 当前源码版本 | `0.5.2`（正式版本） |
| npm `latest` | `0.5.2`（OIDC Trusted Publishing + SLSA v1 provenance） |
...
2. 历史域、人员域、招投标域列为后续二期；按当前产品决定暂不做开发规划或实现。
