...
`0.8.1 企查查智能文档解析图片名单` 已发布至 npm 与 GitHub Release（tag `v0.8.1` / `4d71308`）。图片向导一次生成包含识别、匹配规则、
...
### 2026-09-06 图片接入反馈修复（未提交、未发布）

- 根因：Host 在图片暂存前检查 OCR Provider，失败时 Client 又释放原生附件，导致选图后仍显示“请选择图片”；剪贴板只读取 `files`，未兼容仅 `items` 的截图。
- 已修复：图片校验/暂存与连接状态解耦，保留缩略图及四步向导；`providerReady/providerIssue` 随状态查询更新；缺失连接时执行仍阻断，15 分钟内补齐连接可沿用图片任务。
- 已修复：Composer、向导与拖入统一取图，兼容 `files/items`，不重复取图、不消费纯文本。
- 验证：Host 暂存/连接后恢复、两企业提取、无连接零调用、取消清理；Client 有状态交互测试覆盖文件与剪贴板两条路径，均可预览、下一步并完整回填。测试使用 Mock，不等于本机真实 OCR 验收。
- 待办：修复版本尚未安装至用户 DSH；需发布/更新后进行真实浏览器与本地 `qcc-document-mcp` OCR 验收。当前远端 URL-only 服务不能读取本地图片，不能仅靠本轮 UI 修复解除该依赖。
