...
P1.1～P1.6 开发、验收与发布准备已完成；维护者测试账号的最小真实 Phase-3 E2E 已通过，
仅 `v0.5.0` tag、npm/GitHub 发布仍需用户单独批准。
...
| **P1.6** | 0.5.0 发布准备 | ✅ 完成 | 版本/README/CHANGELOG/兼容/验收/升级回滚、最终 pack、全新 rc.2 隔离安装与最小真实门均已过；等待 tag/OIDC 发布授权 |
...
1. 另行获得用户对 `v0.5.0` tag / push / OIDC 发布的明确批准。
2. 发布完成后核验 npm provenance、GitHub Release 与公共 Registry 全新安装，再把源码状态改为“已发布”。
