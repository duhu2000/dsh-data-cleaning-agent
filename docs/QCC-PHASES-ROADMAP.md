...
| 三期 | 0.5.0 | 方案 A 扩展 Skill：风险/知产/经营 + 方案 B 批量后端 | 风险 38 + 知产 18 + 经营 35 | 同上 + Spike #7（`ctx.loader` 可调） |
...
启动方案 B（插件内 `lib/qcc.js` 直接经 `ctx.loader` 调用 mcp-client 条目）：
- 前置：Spike #7 验证「`ctx.loader` 拿到的 mcp-client 条目可在插件后端直接调用」。
- 若验证失败，方案 B 边界封闭，退回方案 A 的分批模型中介。
